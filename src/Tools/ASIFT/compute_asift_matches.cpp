...
{  	
	...
	{		
		...
		matchings_vec[tt-1] = std::vector< vector< vector < matchingslist > > > (num_rot1);
		Minfoall_vec[tt-1] = std::vector< vector< vector< vector< vector < float > > > > > (num_rot1);
		
		for  ( rr = 1; rr <= num_rot1; rr++ ) 
		{   
			
			matchings_vec[tt-1][rr-1] = std::vector< vector< matchingslist > > (num_tilt2);
			Minfoall_vec[tt-1][rr-1] = std::vector< vector< vector< vector < float > > > > (num_tilt2);
			...
			{			
				...
				matchings_vec[tt-1][rr-1][tt2-1] = std::vector< matchingslist > (num_rot1_2);		
				Minfoall_vec[tt-1][rr-1][tt2-1] = std::vector< vector< vector< float > > > (num_rot1_2);				
			}
		}
	}
...
#pragma omp parallel for private(rr2)
				...
				{   
					...
					if ( matchings1.size() > 0 )
					{
						...
						Minfoall_vec[tt-1][rr-1][tt2-1][rr2-1] = vector< vector<float> >(matchings1.size());
